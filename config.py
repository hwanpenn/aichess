CONFIG = {
    ...
    'batch_size': 1024,  # 每次更新的train_step数量（RTX 3090优化：从512增加到1024，加速训练）
    'kl_targ': 0.02,  # kl散度控制
    'epochs' : 5,  # 每次更新的train_step数量
    'game_batch_num': 3000,  # 训练更新的次数
    'use_frame': 'pytorch',  # paddle or pytorch根据自己的环境进行切换
    'train_update_interval': 600,  #模型更新间隔时间（秒）
    'use_redis': False, # 数据存储方式
    'redis_host': 'localhost',
    'redis_port': 6379,
    'redis_db': 0,
    'show_selfplay': True,  # 是否显示自动对弈过程（True=显示，False=不显示，可节省时间）
    'show_delay': 0.3,  # 每步显示延迟（秒），show_selfplay=True时有效，建议0.3，0=无延迟
}